...
int main()
{
    ...
    {
    bool post_code;
    if((post_code = b.POST(c)) == 0){printf("POST error code--  %d\n", post_code);exit(1);}
    else{printf("POST error code--  %d\n", post_code);}
    }
    b.INIT(c);
    unsigned char test_project[][4] = {{6, 0, 1, 0}, {6, 1, 2, 0}, {9, 0, 1, 0}, {18, 0, 0, 0xf0}, {8, 0, 0, 0}, {16, 0, 0, 0}};
    for (int i = 0; i < sizeof(test_project) / 4; ++i)
    {
        c.mm.mm_write(test_project[i], i * 4);
    }
